...
def radius_function(data):
	return int(data["module_limit"])

def generate_farm_module_owner(data):
	xml = """	<ModOp GUID = '{GUID}' Path="Values/ModuleOwner" Type="replace">
		<ModuleOwner>
			<ModuleLimits>
				<Main>
					<Limit>{module_limit}</Limit>
				</Main>
			</ModuleLimits>
			<ConstructionOptions>
				<Item>
					<ModuleGUID>{module_GUID}</ModuleGUID>
				</Item>
			</ConstructionOptions>
			<ModuleBuildRadius>{radius}</ModuleBuildRadius>
			<HardFarmsConfig>{hard_farms}</HardFarmsConfig>
		</ModuleOwner>				 
